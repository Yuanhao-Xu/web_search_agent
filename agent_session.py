...
请基于搜索结果提供准确、有帮助的回答。"""
    ...
    async def _chat_with_tools(self, force_tool: bool = False) -> str:
        ...
        while self.tool_call_count < self.max_tool_calls:
            # 设置工具选择模式
            tool_choice = "required" if force_tool and self.tool_call_count == 0 else "auto"
    ...
    async def run_interactive(self):
        ...
        print("  /exit     - 退出会话")
        print(f"\n当前设置: 工具模式={self.tool_mode.value}, 最大调用={self.max_tool_calls}次")
        print("="*60 + "\n")
...
class SessionManager:
    ...
    def create_session(self, 
                       ...
                       **kwargs) -> AgentSession:
        """创建新会话"""
        session = AgentSession(api_key=api_key, **kwargs)
...
async def main():
    ...
    session = AgentSession(
        api_key="sk-f5889d58c6db4dd38ca78389a6c7a7e8",  # 替换为你的API key
